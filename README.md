...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Common button styling for both Buy and Preview */
    ...
    /* Preview section now includes a header title */
    #preview h2 {
      font-size: 2.5rem;
      margin-bottom: 2rem;
      text-align: center;
    ...
    /* Toggle Previews Button – attractive styling */
    #togglePreviewsBtn {
      display: block;
      margin: 2rem auto;
...
<body>
  ...
  <div id="sideMenu">
    ...
    <!-- The "Previews" link now simply scrolls to the toggle button -->
    <a href="javascript:void(0)" onclick="scrollToTogglePreviews(); closeMenu();">Previews</a>
  ...
  <script>
    ...
    function performSearch() {
      ...
      results.forEach(item => {
        ...
        resultDiv.innerHTML = `<span>${item.name} - $${item.price}</span>
          ...
          <button type="button" onclick="showOnlyPreview('${item.id}')">Preview</button>`;
    ...
    function showOnlyPreview(itemId) {
      // Force preview section to be shown and override toggle text
    ...
    // Function to scroll to the "Show Previews" toggle button
    function scrollToTogglePreviews() {
      const toggleBtn = document.getElementById("togglePreviewsBtn");
